    ...
    def eliminar_postres_repetidos(self):
        ...
        for p in self.arreglo_postres:
            if p.nombre.lower() not in nombres_vistos:
                ...
                conteo += 1
        self.arreglo_postres = nueva_lista
        print(f"Limpieza terminada: Se eliminaron {conteo} postres duplicados.")

    def eliminar_ingredientes_repetidos(self):
        for postre in self.arreglo_postres:
            actual = postre.lista_ingredientes
            while actual:
                corredor = actual
                while corredor.siguiente:
                    if corredor.siguiente.nombre.lower() == actual.nombre.lower():
                        corredor.siguiente = corredor.siguiente.siguiente
                    else:
                        corredor = corredor.siguiente
                actual = actual.siguiente
        print("Limpieza terminada: Se quitaron ingredientes repetidos en cada postre.")
...
def menu():
    ...
    while True:
        ...
        print("6. LIMPIAR POSTRES REPETIDOS")
        print("7. LIMPIAR INGREDIENTES REPETIDOS")
        print("8. Salir")
        ...
        elif op == "6": sys.eliminar_postres_repetidos()
        elif op == "7": sys.eliminar_ingredientes_repetidos()
        elif op == "8": break
