"""
╔══════════════════════════════════════════════════════════════════════════╗
║   VISUALIZACIÓN DE ALGORITMOS DE ORDENAMIENTO EXTERNO  v2.0             ║
║   1. Intercalación   2. Mezcla Directa   3. Mezcla Equilibrada          ║
║   ── CON ARCHIVO 1, ARCHIVO 2 Y SELECCIÓN DE HOJAS DE EXCEL ──          ║
╚══════════════════════════════════════════════════════════════════════════╝

Formatos de archivo soportados para carga de datos:
  • .txt   → Números separados por espacios, comas o saltos de línea
  • .csv   → Primera columna numérica, o columna elegida por el usuario
  • .json  → Array plano o array de objetos
  • .xml   → Etiquetas <value>, <item>, <number> o <data>
  • .yaml  → Lista plana o bajo una clave
  • .xlsx / .xls → Muestra un menú para seleccionar qué hojas leer
...
SUPPORTED_EXTENSIONS = {
    ".txt":  "Texto plano (números separados por espacios/comas)",
    ".csv":  "CSV (comma-separated values)",
    ".json": "JSON (array o array de objetos)",
    ".xml":  "XML (etiquetas <value>, <item>, <number>)",
    ".yaml": "YAML (lista plana o bajo una clave)",
    ".yml":  "YAML (alias de .yaml)",
    ".xlsx": "Excel 2007+ (requiere openpyxl)",
    ".xls":  "Excel 97-2003 (requiere xlrd)",
    ".npy":  "NumPy array binario",
}
...
def ventana_seleccion_hojas(hojas: list[str]) -> list[str]:
    ...
    vars_check = []
    for h in hojas:
        var = tk.IntVar(value=1) # Marcadas por defecto
...
def cargar_archivo(ruta: str) -> list[int]:
    """
    Lee un archivo y extrae una lista de enteros.
    Lanza ValueError si no puede parsear o si el resultado está vacío.
    """
    ...
    if ext == ".txt":
        ...
        if not nums: raise ValueError("No se encontraron números en el archivo .txt")
        ...
        with open(ruta, encoding="utf-8") as f:
            ...
            for fila in reader:
                for celda in fila:
                    try: resultados.append(int(float(celda.strip())))
                    except ValueError: pass
        if not resultados: raise ValueError("No se encontraron valores numéricos en el .csv")
        return resultados

    elif ext == ".json":
        with open(ruta, encoding="utf-8") as f:
            data = json.load(f)
        nums = _extraer_numeros_json(data)
        if not nums: raise ValueError("No se encontraron números en el .json")
        return nums
        ...
        root = tree.getroot()
        nums = []
        for tag in ("value", "item", "number", "data", "val", "num"):
            for elem in root.iter(tag):
                try: nums.append(int(float(elem.text.strip())))
                except (TypeError, ValueError): pass
        if not nums:
            texto = ET.tostring(root, encoding="unicode")
            nums = [int(float(n)) for n in re.findall(r"-?\d+(?:\.\d+)?", texto)]
        if not nums: raise ValueError("No se encontraron números en el .xml")
        return nums
        ...
        nums = _extraer_numeros_json(data)
        if not nums: raise ValueError("No se encontraron números en el .yaml")
        return nums

    # ── XLSX con selección de Hojas ──────────────────────────────────────────
    elif ext == ".xlsx":
        try: import openpyxl
        ...
        seleccionadas = hojas
        if len(hojas) > 1:
            seleccionadas = ventana_seleccion_hojas(hojas)
            ...
            for fila in ws.iter_rows(values_only=True):
                for celda in fila:
                    if isinstance(celda, (int, float)):
                        nums.append(int(celda))
        wb.close()
        if not nums: raise ValueError("No se encontraron números en las hojas seleccionadas (.xlsx)")
        return nums

    # ── XLS con selección de Hojas ───────────────────────────────────────────
    elif ext == ".xls":
        try: import xlrd
        ...
        for h in seleccionadas:
            ...
            for r in range(ws.nrows):
                for c in range(ws.ncols):
                    cell = ws.cell(r, c)
                    if cell.ctype in (2, 3):   # 2=número, 3=fecha
                        nums.append(int(cell.value))
        if not nums: raise ValueError("No se encontraron números en las hojas seleccionadas (.xls)")
        return nums

    elif ext == ".npy":
        arr = np.load(ruta, allow_pickle=False).flatten()
...
def _extraer_numeros_json(data) -> list[int]:
    nums = []
    if isinstance(data, list):
        for item in data:
            if isinstance(item, (int, float)): nums.append(int(item))
            elif isinstance(item, dict):
                for v in item.values():
                    if isinstance(v, (int, float)):
                        nums.append(int(v)); break
            elif isinstance(item, list): nums.extend(_extraer_numeros_json(item))
    elif isinstance(data, dict):
        for v in data.values(): nums.extend(_extraer_numeros_json(v))
    return nums


def abrir_dialogo_archivo() -> tuple[list[int] | None, str]:
    """Abre un diálogo de selección múltiple y retorna (datos_combinados, nombres_archivos)."""
    root = Tk()
    root.withdraw()
    root.attributes("-topmost", True)
    ...
    rutas = filedialog.askopenfilenames(
        title="Seleccionar archivo(s) de datos",
        filetypes=tipos
    )
    root.destroy()
...
def draw_array(ax, data, colors, y=0, cell_h=0.7, label="", title_color=TEXT,
               cell_w=0.9, fontsize=11):
    n = len(data)
    for i, (val, col) in enumerate(zip(data, colors)):
        rect = FancyBboxPatch((i * cell_w, y), cell_w - 0.05, cell_h,
                              boxstyle="round,pad=0.05",
                              fc=col, ec=BORDER, lw=1.2)
        ax.add_patch(rect)
        ax.text(i * cell_w + (cell_w - 0.05) / 2, y + cell_h / 2, str(val),
                ha="center", va="center",
                fontsize=fontsize, fontweight="bold",
                color=BG if col != PANEL else TEXT)
    if label:
        ax.text(-0.6, y + cell_h / 2, label,
                ha="right", va="center", fontsize=9,
                color=title_color, fontweight="bold")
    return n

def section_title(ax, text, color, fontsize=13):
    ax.set_title(text, color=color, fontsize=fontsize, fontweight="bold",
                 pad=10, loc="left",
...
def normalizar_datos(datos: list[int], max_n: int = 60) -> list[int]:
    """Trunca para no colapsar la vista visual en combinaciones gigantes."""
    if len(datos) > max_n:
        datos = datos[:max_n]
    return datos
...
# ══════════════════════════════════════════════════════════════════════════════
#  ALGORITMO 1 – INTERCALACIÓN
# ══════════════════════════════════════════════════════════════════════════════

def demo_intercalacion(ax, datos: list[int]):
    n2 = len(datos) // 2
    A = sorted(datos[:n2])
    B = sorted(datos[n2:])
    merged_so_far = []
    ...
    def render(frame):
        ...
        ax.text(0, 5.1,
                f"Problema: Dividir en 2 mitades ya ordenadas y fusionarlas.\n"
                f"Datos cargados: {len(datos)} elementos  |  Mitad A: {len(A)}  Mitad B: {len(B)}",
        ...
        if si < len(steps):
            ...
            ax.annotate("", xy=(xi, 3.2), xytext=(xi, 2.0 + 0.7),
                        arrowprops=dict(arrowstyle="<->", color=GOLD, lw=1.5))

        col_res = [MERGE_C] * len(merged_so_far) + [PANEL] * (len(A) + len(B) - len(merged_so_far))
        draw_array(ax, merged_so_far + [0] * (len(A) + len(B) - len(merged_so_far)),
    ...
    while size < n:
        ...
        for left in range(0, n, size * 2):
            ...
            while i2 < len(L) and j2 < len(R):
                if L[i2] <= R[j2]:
                    merged.append(L[i2]); i2 += 1
                else:
                    merged.append(R[j2]); j2 += 1
            merged += L[i2:] + R[j2:]
            nueva += merged
        arr = nueva
        estados.append(arr[:])
...
def demo_mezcla_directa(ax, datos: list[int]):
    ...
    def render(frame):
        ...
        ax.text(0, len(estados) * 1.5 + 1.6,
                f"Cada pasada duplica el tamaño de los bloques ordenados.\n"
                f"Datos: {len(datos)} elementos  |  Pasadas necesarias: {len(estados)-1}",
                fontsize=8, color=SUBTEXT, va="top")
        ...
        for p, estado in enumerate(estados[:pasada_actual + 1]):
            ...
            cols = [block_colors[(k // (2 ** p if p > 0 else 1)) % len(block_colors)]
                    for k in range(len(estado))]
            draw_array(ax, estado, cols, y=y_pos, label=label_p,
                       title_color=ACCENT2, cell_w=cw)

        size = 2 ** pasada_actual if pasada_actual > 0 else 1
        ax.text(0, -0.6,
                f"Pasada {pasada_actual}: bloques de tamaño {size}  →  {len(estados)-1} pasadas totales",
...
# ══════════════════════════════════════════════════════════════════════════════
#  ALGORITMO 3 – MEZCLA EQUILIBRADA
# ══════════════════════════════════════════════════════════════════════════════

def demo_mezcla_equilibrada(ax, datos: list[int]):
    k = 2
    run_size = 2
    runs = [sorted(datos[i:i+run_size]) for i in range(0, len(datos), run_size)]
    cintas_in = [[] for _ in range(k)]
    for idx, run in enumerate(runs):
        cintas_in[idx % k].append(run)
    ...
    for _ in range(8):
        ...
        for idx2, mr in enumerate(merged_runs):
            cintas_out[idx2 % k].append(mr)
        pasadas.append({"in": [list(c) for c in fuente], "out": [list(c) for c in cintas_out],
                        "label": f"Pasada {len(pasadas)}"})
        fuente = cintas_out
    ...
    def render(frame):
        ...
        ax.text(0, 6.6,
                f"Distribuye runs en k=2 cintas (archivos) y mezcla por pasadas.\n"
                f"Datos: {len(datos)} elementos  |  Runs iniciales: {len(runs)}",
                fontsize=8, color=SUBTEXT, va="top")
        ...
        for ci, cinta in enumerate(pasada["in"]):
            y_c = 4.5 - ci * 1.3
            ax.text(-0.4, y_c + 0.35, f"C{ci+1}", fontsize=9,
                    color=colors_cinta[ci], fontweight="bold", ha="right")
            x_off = 0
            for run in cinta:
                for val in run:
                    rect = FancyBboxPatch((x_off, y_c), 0.8, 0.65, boxstyle="round,pad=0.04",
                                          fc=colors_cinta[ci], ec=BORDER, lw=1)
                    ax.add_patch(rect)
                    ax.text(x_off + 0.4, y_c + 0.325, str(val), ha="center", va="center", fontsize=8, fontweight="bold", color=BG)
                    x_off += 0.85
        ...
        if pasada.get("out"):
            ...
            for ci, cinta in enumerate(pasada["out"]):
                ...
                for run in cinta:
                    for val in run:
                        rect = FancyBboxPatch((x_off, y_c), 0.8, 0.65, boxstyle="round,pad=0.04",
                                              fc=MERGE_C, ec=BORDER, lw=1)
                        ax.add_patch(rect)
                        ax.text(x_off + 0.4, y_c + 0.325, str(val), ha="center", va="center", fontsize=8, fontweight="bold", color=BG)
                        x_off += 0.85
                    x_off += 0.35
        else:
            ax.text(2, 1.5, "← Distribuyendo runs iniciales...", fontsize=9, color=SUBTEXT)
...
def vista_comparacion(fig, datos: list[int], seleccionados: list[bool]):
    fig.clear()
    ...
    for spine in sum([list(a.spines.values()) for a in axes], []):
    ...
    for btn in (btn_prev, btn_play, btn_next, btn_rst, btn_back):
        btn.label.set_color(TEXT); btn.label.set_fontfamily("monospace")
    ...
    def on_prev(_):
        state["playing"] = False; btn_play.label.set_text("▶ Reanudar")
        state["frame"] = max(0, state["frame"] - 1)
        redraw()

    def on_play(_):
        state["playing"] = not state["playing"]
        btn_play.label.set_text("⏸ Pausar" if state["playing"] else "▶ Reanudar")
        fig.canvas.draw_idle()
    ...
    def on_rst(_):
        ...
        btn_play.label.set_text("⏸ Pausar"); redraw()
    ...
    btn_rst.on_clicked(on_rst)
    btn_back.on_clicked(on_back)

    def update(_):
        if state["playing"]:
            if state["frame"] < max_frames - 1:
                state["frame"] += 1; redraw()
            else:
                state["playing"] = False; btn_play.label.set_text("▶ Reanudar")
                fig.canvas.draw_idle()

    timer = fig.canvas.new_timer(interval=700)
...
def main():
    datos_default = [45, 12, 67, 23, 89, 34, 56, 78, 91, 5]
    
    # Manejamos los dos archivos (slots) que pidió el usuario
    datos_archivos = {
        "1": {"valores": [], "nombre": ""},
        "2": {"valores": [], "nombre": ""}
    }
    datos_actuales = {"valores": datos_default[:], "nombre": "datos por defecto"}

    fig = plt.figure(figsize=(15, 9), facecolor=BG)
    fig.canvas.manager.set_window_title("Visualizador de Ordenamiento Externo  v2.0")
    ...
    # Reubicación de la barra de botones
    ...
    # Nuevos botones de Archivo 1 y Archivo 2
    ...
    fig.text(0.09, 0.85, "Visualizar:", ha="center", va="top", fontsize=8, color=SUBTEXT, fontfamily="monospace")
    ...
    for lbl, col in zip(radio.labels, colors_btn):
        lbl.set_color(col); lbl.set_fontfamily("monospace"); lbl.set_fontsize(9); lbl.set_fontweight("bold")
    ...
    for lbl, col in zip(check.labels, colors_btn):
        lbl.set_color(col); lbl.set_fontfamily("monospace"); lbl.set_fontsize(8); lbl.set_fontweight("bold")

    state = {"selected": 0, "frame": 0, "playing": True, "render_fn": None, "max_frames": 1}
    descriptions = ["INTERCALACIÓN\n\nFusiona 2 mitades ya\nordenadas en una\nsola pasada lineal.\n\nComplejidad: O(n)", "MEZCLA DIRECTA\n\nOrdena con runs\ncrecientes (×2 por\npasada).\n\nComplejidad: O(n log n)", "MEZCLA EQUILIBRADA\n\nDistribuye en k=2\ncintas alternas y\nmezcla por pasadas.\n\nO(n log n)"]
    ...
    def actualizar_info(idx):
        ...
        # Muestra qué archivos están combinados
        texto_fuente = datos_actuales['nombre']
        if len(texto_fuente) > 25:
            texto_fuente = texto_fuente[:22] + "..."
            
        resumen = f"Datos Totales: {len(d)}\n"
        resumen += f"Fuente: {texto_fuente}\n"
        if len(d) > 0:
            resumen += f"Min: {min(d)}  Max: {max(d)}\n\n"
        ax_info.text(0.05, 0.98, resumen + descriptions[idx],
                     transform=ax_info.transAxes, va="top", fontsize=7.5, color=TEXT, fontfamily="monospace",
                     path_effects=[pe.withStroke(linewidth=2, foreground=BG)])
        fig.canvas.draw_idle()

    def cargar_demo(idx):
        datos = normalizar_datos(datos_actuales["valores"]) # Mantiene máx 60 por rendimiento visual
        if not datos: return
    ...
    btn_next    = Button(ax_next,  "Next ▶",       color=PANEL, hovercolor=BORDER)
    btn_rst     = Button(ax_rst,   "↺ Reiniciar",  color=PANEL, hovercolor=BORDER)
    btn_arch1   = Button(ax_arch1, "📂 Arch 1",    color="#182b40", hovercolor="#254261") # Azul oscuro
    btn_arch2   = Button(ax_arch2, "📂 Arch 2",    color="#1f3a2b", hovercolor="#2d5a3d") # Verde oscuro
    ...
    btn_arch1.label.set_color(ACCENT1)
    btn_arch2.label.set_color(ACCENT2)
    btn_comp.label.set_color(ACCENT3)

    def on_prev(_):
        state["playing"] = False; btn_play.label.set_text("▶ Reanudar")
        state["frame"] = max(0, state["frame"] - 1); state["render_fn"](state["frame"]); fig.canvas.draw_idle()

    def on_play(_):
        state["playing"] = not state["playing"]; btn_play.label.set_text("⏸ Pausar" if state["playing"] else "▶ Reanudar")
        fig.canvas.draw_idle()

    def on_next(_):
        state["playing"] = False; btn_play.label.set_text("▶ Reanudar")
        state["frame"] = min(state["frame"] + 1, state["max_frames"] - 1); state["render_fn"](state["frame"]); fig.canvas.draw_idle()

    def on_rst(_):
        state["frame"] = 0; state["playing"] = True; btn_play.label.set_text("⏸ Pausar"); state["render_fn"](0); fig.canvas.draw_idle()

    def agrupar_datos_globales():
        """Une los datos de Archivo 1 y Archivo 2 en un solo conjunto"""
        comb = []
        noms = []
        if datos_archivos["1"]["valores"]:
            comb.extend(datos_archivos["1"]["valores"]); noms.append("A1:"+datos_archivos["1"]["nombre"])
        if datos_archivos["2"]["valores"]:
            comb.extend(datos_archivos["2"]["valores"]); noms.append("A2:"+datos_archivos["2"]["nombre"])
            
        if comb:
            datos_actuales["valores"] = comb
            datos_actuales["nombre"] = " + ".join(noms)
        else:
            datos_actuales["valores"] = datos_default[:]
            datos_actuales["nombre"] = "datos por defecto"
    ...
    def cargar_slot(slot):
        ...
        ax_vis.text(0.5, 0.02, f"✔ Archivo {slot} cargado exitosamente. Datos combinados.",
                    ha="center", va="bottom", fontsize=8, color=ACCENT2, transform=ax_vis.transAxes, fontfamily="monospace")
        fig.canvas.draw_idle()
    ...
    btn_next.on_clicked(on_next)
    btn_rst.on_clicked(on_rst)
    btn_arch1.on_clicked(on_arch1)
    btn_arch2.on_clicked(on_arch2)
    btn_comp.on_clicked(on_comparar)

    ax_prog = fig.add_axes([0.21, 0.10, 0.77, 0.02], facecolor=BG)
    ax_prog.axis("off")
    ...
    def update(_):
        if state["playing"] and state["render_fn"]:
            if state["frame"] < state["max_frames"] - 1:
                state["frame"] += 1; state["render_fn"](state["frame"])
            else:
                state["playing"] = False; btn_play.label.set_text("▶ Reanudar")
